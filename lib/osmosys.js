...
function getStateSlice(path, state) {
  return isUnnamedIsolation(path) ? view(lensPath(split('.', path)), state) : state
}
...
class Osmosys {
  ...
  select(path) {
    ...
    const _selectObject = this._selectObject.bind(this)
    const _select = this._select.bind(this)
    return ifElse(is(Object), _selectObject, _select)(path)
  }
